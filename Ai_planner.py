...
def travel_option(source,destination,mode,travel_date):
    ...
    try :
        response = model.invoke([user_prommpt,system_prompt])
        return response.content if response else 'No Responce'
...
if sp.button('Find Traveling options'):
    if source.strip() and destination.strip():
        result = travel_option(source,destination,mode,travel_date)
        sp.markdown(result)
    else:
